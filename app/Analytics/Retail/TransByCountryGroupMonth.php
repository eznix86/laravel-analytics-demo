...
/**
 * A third rollup of the same two metrics, along a derived dimension.
 */
class TransByCountryGroupMonth extends Model implements AnalyticsModel
{
    use Analytics;
    ...
    public function computes(): Query
    {
        return $this->from(SemanticTransaction::class, 't')
            ->per(date_trunc('month', 't.created_at')->as('created_at_month'), 't.country_group')
